...
from auralogs.config import DEFAULT_MAX_QUEUE_SIZE, AuralogsConfig


def test_config_defaults():
    cfg = AuralogsConfig(api_key="k")
    assert cfg.api_key == "k"
    assert cfg.environment == "production"
    assert cfg.endpoint == "https://ingest.auralogs.ai"
...
def test_config_environment_override():
    cfg = AuralogsConfig(api_key="k", environment="staging")
...
def test_config_rejects_http_endpoint_by_default():
    with pytest.raises(ValueError, match="https://"):
        AuralogsConfig(api_key="k", endpoint="http://insecure")


def test_config_accepts_http_endpoint_with_opt_in():
    cfg = AuralogsConfig(api_key="k", endpoint="http://insecure", allow_insecure_endpoint=True)
...
def test_config_max_queue_size_override():
    cfg = AuralogsConfig(api_key="k", max_queue_size=42)
...
def test_config_rejects_zero_max_queue_size():
    ...
    with pytest.raises(ValueError, match="max_queue_size"):
        AuralogsConfig(api_key="k", max_queue_size=0)


def test_config_rejects_negative_max_queue_size():
    with pytest.raises(ValueError, match="max_queue_size"):
        AuralogsConfig(api_key="k", max_queue_size=-1)


def test_config_accepts_uppercase_https_scheme():
    # Per RFC 3986 §3.1 URI schemes are case-insensitive.
    cfg = AuralogsConfig(api_key="k", endpoint="HTTPS://ingest.example.com")
